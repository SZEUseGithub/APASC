...
global_step = 0;
%**********計算中強度運動時長**********
moderate_intensity_Time = 0;low_intensity_Time = 0;
%**************************************
...
%         [1];
        ...
        [winStep, winFreq, winTime] = eff_euler_online_step_count_1(bin, Eul, GlbCnt, phyBgnBin, phyEndBin, bgnBin, endBin);
        
%         fprintf('cwin = %d  nbin = %d  maxCompBin=%d  winStep=%d\n', cwin, endBin, maxCompBin, winStep);
        fprintf('cwin = %d  nbin = %d  maxCompBin=%d winFreq=%.2f winTime=%5.2f winStep=%d\n', cwin, endBin, maxCompBin, winFreq, winTime, winStep);
...
%**********計算中強度運動時長**********
        if (winFreq>1.67)            
            moderate_intensity_Time = moderate_intensity_Time + winTime;
        else
            low_intensity_Time = low_intensity_Time + winTime;
        end
%**************************************
    end
end % while (1) t loop nwrk ib
fprintf('global_step = %d\n',global_step);
fprintf('moderate-intensity walking = %.2f sec  low-intensity walking = %.2f sec\n',moderate_intensity_Time, low_intensity_Time);
...
function [winStep, winFreq, winTime] = eff_euler_online_step_count_1(nbin, Eul0, GlbCnt, phyBgnBin, phyEndBin, bgnBin, endBin)
...
[winStep, winFreq, winTime] = process_each_window_Euler_only_0625(GlbCnt, numBnd, LeftBnd, RightBnd, pitch1, pitch2, phyBgnBin, phyEndBin);
...
function [winStep, winFreq, winTime] = process_each_window_Euler_only_0625(GlbCnt, numSeg, LeftBnd, RightBnd, pitch1, pitch2, phyBgnBin, phyEndBin)
...
for (s=1:numSeg)
    ...
    %**********計算中強度運動時長**********怪
%     binBgn = min(phyEndBin,binBgn);
%     binEnd = max(phyBgnBin,binEnd);
    %**************************************
    [segStep, segPeriod] = segment_step_by_integration_0609_2016(GlbCnt, binBgn, binEnd, pitch1, pitch2); % physical domain 的 wd(bgn) wd(end)=?
    
    winStep = winStep + segStep; 
    %**********計算中強度運動時長**********
    winFreq = winFreq + segPeriod;
    winTime = winTime + binEnd-binBgn+1;
    %**************************************
...
function  [sumStep, SumP2] = segment_step_by_integration_0609_2016(GlbCnt, binBgn, binEnd, pitch1, pitch2)
